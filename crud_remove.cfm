...
<cfif NOT listLen( arguments.lFilterFields )>
	<cfset request.log( "!!Error: No fields to filter by. [RemoveCrud][#sTableName#]" )>
	<cfreturn>
	<cfthrow type="Custom.CFC.CrudGen.RemoveCrud.NoFilterFields"
...
<cfif listLen( arguments.lFilterFields )>
	<cfset this.appendCommentLine( "Remove a single record from #arguments.sTableName#" )>
	<cfset this.appendCommentLine( "based on fields: #replace( arguments.lFilterFields, ',', ', ', 'all' )#" )>
<cfelse>
	<cfset this.appendCommentLine( "Remove ALL records from #arguments.sTableName#" )>
</cfif>
<cfset this.appendDivide()>
<cfset this.appendBlank()> --->
...
<cfset this.addDefinition( "remove", arguments, this.readBuffer( arguments.sCrudName ) )>
